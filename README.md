...
For managing files I have used gemini file api for handling different file formats. as Gemini API accepts file size greater than 100mb. So it's more efficient.
...
uri and mimetype are needed for createUserContent method of gemini.
...
**Step 1)** user should send a post `/api/user/ai-task-run/:id` request
or `/api/user/ai-task-run/others` for a new task that doesn't exist in the DB
(with valid inputs to get a roomId)

**Step 2)** User connects to socket.io request on postman on url `http://localhost:3000`
